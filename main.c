...
#define TEXT_MAKS_SIZE 4096  /** Max symbols in line **/
#define STOP 7  /** Stop symbol size **/
#define FILE_NAME_MAKS 32 /** File's name max size **/
#define SETTINGS_NAME "settings.letitop"  /** Settings file name **/

void entrance(long *fontsize, long *fieldsize);  /** Main menu of the program **/
void settings(long *fontsize, long *fieldsize);  /** Preferences Function **/
void format(char *a); /** Deleting line transfers at the end of the string **/
void put_in_settings (long const *fontsize, long const *fieldsize); /** Function used to put long type variables into settings file **/
void get_from_settings (long *fontsize, long *fieldsize); /** Reading variables values from settings file **/
void WritePut(char const *all_text, int const *line, long const *fontsize, long const *fieldsize, int mode); /** Function used to write result into a file **/
void ConsoleI(long const *fontsize, long const *fieldsize); /** Function used to get input from console **/
void FileI(long const *fontsize, long const *fieldsize); /** Function used to get input from the file **/
...
void WritePut(char const *all_text, int const *line, long const *fontsize, long const *fieldsize, int mode) {
    ...
    if (mode == 2) {
        ...
        scanf("%s", file_name); /** Get file name from user input **/
        fflush(stdin);
...
void ConsoleI(long const *fontsize, long const *fieldsize){
    ...
    if (dy_text == NULL) {
        ...
        while (mode != 1 && mode != 2) { scanf("%d", &mode); fflush(stdin);} /** Determine where to put the result **/
...
void FileI(long const *fontsize, long const *fieldsize) {
    ...
    if (dy_text == NULL) {
        ...
        while (mode != 1 && mode != 2) {scanf("%d", &mode); fflush(stdin); }
        printf("Enter the file name with source text:\n");
        scanf("%s", file_name);
        fflush(stdin);
        line = 0;
        fp = fopen(file_name, "r");
        if (fp == NULL) {
            printf("\nError Occurred While Trying To read the file %s. Maybe that file does not exist?\n", file_name);
            exit(1);
        } else {
            ...
            /**bThe WritePut() will now work if not all the lines have line transfer symbol at the end. **/
